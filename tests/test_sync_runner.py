...
async def test_run_sync_continues_when_message_store_upsert_fails(
    ...
    # Should NOT raise — best-effort store contract: sync proceeds even when
    # the upsert fails. The background ExtractionWorker has nothing to do
    # this tick (no row landed in channel_messages) but the next sync will
    # retry the upsert.
    await runner._run_sync(
        job_id="job-4",
        channel_id="C_FAIL",
...
# ---------------------------------------------------------------------------
# Sync never invokes BatchProcessor inline — the worker owns extraction.
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_run_sync_never_invokes_batch_processor_inline(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """``_run_sync`` MUST never call ``BatchProcessor.process_messages``.

    Messages land in ``channel_messages`` via the upsert with
    ``extraction_status="pending"``; the background ExtractionWorker is
    the only path that runs the LLM pipeline. Sync completes in seconds
    ...
    assert process_called["ran"] is False, (
        "_run_sync must never invoke BatchProcessor.process_messages — "
